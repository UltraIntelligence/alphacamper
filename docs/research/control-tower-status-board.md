...
|---|---|---|---|
| Linnaeus | High | Worker reliability gate: Railway heartbeat and alert-pipeline unblock | Reported yellow/blocked; Railway/operator access required; no patch |
| Darwin | High | Revenue gate: Stripe production checkout, webhook, and net proof | Reported yellow; checkout webhook proof hardening landed in `28336c0a6` |
| Epicurus | High | Product moat and next epic queue | Reported yellow; next goal-window order confirmed |
...
- Latest verified worker smoke at 2026-05-09T15:12:56Z remains yellow: active watches 5, total alerts 0, delivered alerts 0, `missing_worker_heartbeat`, no Supabase heartbeat, and missing platforms `bc_parks`, `ontario_parks`, `parks_canada`, `gtc_new_brunswick`, and `recreation_gov`.
...
3. Demand Capture And Conversion: operator queue is now built in the revenue-quality view, the live route exposes the new demand response shape, `npm run smoke:demand` has proven controlled request -> aggregate -> cleanup, and production has `OPERATOR_EMAIL_ALLOWLIST`. Use an approved operator token with `smoke:demand` to verify the protected operator API/dashboard rendering before calling the lane fully green. Do not count it as revenue or reliability.
